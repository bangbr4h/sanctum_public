...
class VireaSanctum(QWidget):
    ...
    def initUI(self):
        ...
        if pixmap.isNull():
            ...
            portrait.setToolTip("Portrait image not found.")
            pixmap = QPixmap(100, 100)
            pixmap.fill(Qt.gray)
            portrait.setText("Virea")
            portrait.setAlignment(Qt.AlignCenter)
            portrait.setStyleSheet("color: white; font-size: 12px;")
        else:
            pixmap = pixmap.scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            portrait.setPixmap(pixmap)
            portrait.setToolTip("Virea Operational Visual")
